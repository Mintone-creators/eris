...
class Client extends EventEmitter {
    ...
    async searchGuildMembers(guildID, query, limit) {
        const members = await this.requestHandler.request("GET", Endpoints.GUILD_MEMBERS_SEARCH(guildID), true, {
            query,
            limit
        });
        const guild = this.guilds.get(guildID);
     ...
     * @param {String} channelID The ID of the voice channel
     * @param {String} status The status of the voice channel
     * @param {String} [reason] The reason to be displayed in audit logs
     * @returns {Promise}
     */
    ...
    async syncGuildTemplate(guildID, code) {
        const template = await this.requestHandler.request("PUT", Endpoints.GUILD_TEMPLATE_GUILD(guildID, code), true);
